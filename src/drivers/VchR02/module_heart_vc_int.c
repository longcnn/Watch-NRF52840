...
*---------------------------------------------------------------------------------------------------------------------
		...
		数的代码，在函数‘vcHr02WriteRegisters(uint8_t startAddress, uint8_t *pRegisters, uint8_t len)’添加I2C的写函数的
...
#define GH30X_RESET_PIN_NUMBER (30U)
#define GH30X_INT_PIN_NUMBER (29U)
#define GH30x_ADDR 0x66 // !< Device address in bits [7:1]   
...
#define TWI_DONT_ISSUE_STOP ((bool)false) //!< Parameter for @ref twi_master_transfer
 uint8_t maxDataSize = 8;
 uint8_t registerSize= 1;
 uint8_t internalBuffer[9];
...
const uint32_t mcuOscData = 1024;
...
void vcHr02Init(vcHr02_t *pVcHr02,vcHr02Mode_t vcHr02WorkMode)
{
	...
	else if(pVcHr02->workMode ==  VCWORK_MODE_TEMPWORK)
	{
		vcHr02StartSample(pVcHr02);
	}
			...
			if(vcHr02.vcFifoReadFlag || vcHr02.vcPsFlag)
			{
				...
				*/
				...
				for(uint8_t i=0;i<20;i++)
				{
					...
					//右移5位的操作是心率算法需要的三轴的数据是256代表1g(1个重力加速度)
				...
				if(vcHr02.vcFifoReadFlag)
				{
					...
					{					
						...
						{					
							...
							checktemp= true;
							//SEGGER_RTT_printf(0, "vc=%d - real=%d -leng=%d - Ir=%d -Re=%d -Flag=%d\n",(int)vcSpo2Value,(int)real_spo,(int)ppgLength ,(int)vcIrPPG,(int)vcRedPPG ,(int)vcSportFlag );
...
void SpO2Int(){
	vcHr02Mode_t vcMode = VCWORK_MODE_SPO2WORK;
	//vcHr02PhyInit();      	
    vcHr02Init(&vcHr02,vcMode);
}
...
************************************************************************************/
  ...
  internalBuffer[0] = startAddress;
  memcpy(internalBuffer+1,pRegisters, len);
 return  !gh30x_twi_master_transfer(GH30x_ADDR,internalBuffer, len+1, TWI_ISSUE_STOP);
...
/*------------------INSERT YOUR CODE HERE-----------------*/
 uint8_t ret = 0;
 ret = gh30x_twi_master_transfer(0x66,&startAddress, 1, TWI_DONT_ISSUE_STOP);
 ret &= gh30x_twi_master_transfer(0x67| TWI_READ_BIT, pRegisters, len, TWI_ISSUE_STOP);
 nrf_delay_us(40);
 return !ret;
}
...
static void gh30x_int_pin_handler(nrf_drv_gpiote_pin_t pin, nrf_gpiote_polarity_t action)
{
    ...
    {
         vcHr02IRQFlag = true;
		// SEGGER_RTT_printf(0, " vcHr02IRQFlag\n");
...
//SEGGER_RTT_printf(0, "vc=%d\n",(int)vcSpo2Calculate(12.55f,56.354f));
//vcHr02ReadRegisters(VCREG17,vcRegisterValue,6); 
//vcHr02ReadRegisters(VCREG17,vcRegisterValue,6);	
}
