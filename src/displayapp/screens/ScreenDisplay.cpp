...
#include "displayapp/icons/clock/helpSmall.c"
...
*/
        ...
        case Modes::CheckIn:
          ...
          */
          ...
          buttonSmall =  lv_icon_create(lv_scr_act(), &helpSmall);  
          lv_obj_align(buttonSmall, NULL, LV_ALIGN_IN_TOP_RIGHT, 0, 27);
        ...
        case Modes::Charging:
          batteryController.setGoToSleep(true);  
          arcCharging = lv_arc_create(lv_scr_act(), NULL);  
          // make a circle gray color
          lv_arc_set_bg_start_angle(arcCharging, 0);
          lv_arc_set_bg_end_angle(arcCharging, 360);           
          lv_arc_set_rotation(arcCharging, 270); // rotate origin
          lv_arc_set_start_angle(arcCharging, 0);
          lv_obj_set_size(arcCharging, 160, 160);
          lv_obj_align(arcCharging, NULL, LV_ALIGN_CENTER, 0, 10);
          timeoutCountStart = xTaskGetTickCount(); 
          /* Make label */
        ...
        case Modes::Temp:
          ...
          */
          ...
          textline2 = lv_label_create(lv_scr_act(), NULL);
          lv_obj_set_style_local_text_font(textline2, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, &San_Francisco_18); 
          lv_label_set_text(textline2, "Wrist Temperature is Lower than Forehead"); 
          lv_obj_align(textline2, NULL, LV_ALIGN_CENTER, 0, 85); 
        ...
        if((mode==Modes::Clock)||(mode==Modes::Test)||(mode==Modes::Impact)||(mode==Modes::Fall)||(mode==Modes::CheckIn)){
         ...
         // lv_obj_set_style_local_line_width(buttonSmallCircle, LV_ARC_PART_INDIC, LV_STATE_DEFAULT, LV_DPX(6));
          ...
          lv_arc_set_bg_end_angle(buttonSmallCircle, 0);
          lv_arc_set_start_angle(buttonSmallCircle,0 );   
          //lv_arc_set_end_angle(buttonSmallCircle, 90);  
          lv_obj_set_size(buttonSmallCircle, 74, 74);
          lv_arc_set_rotation(buttonSmallCircle, 90); // rotate origin     
          lv_obj_align(buttonSmallCircle, NULL, LV_ALIGN_IN_TOP_RIGHT, 2, 26);
          lv_obj_set_hidden(buttonSmallCircle, true);
        }                  
 }
...
ScreenDisplay::~ScreenDisplay() {
  ...
  checkTouchSmallButton = false;
  checkPolling = true;
  enableTouchBigButton= false;
  enableTouchSmallButton = false;
  //tempSensor.stop();
}
...
/*
    if((mode!=Modes::Clock)&&(mode!=Modes::Test)&&(mode!=Modes::Impact)&&(mode!=Modes::Fall)&&(mode!=Modes::CheckIn)) {                 
        if((modPolling>200) && checkPolling) {
          app->SetTouchMode(DisplayApp::TouchModes::Gestures);
          checkPolling = false;
        } else modPolling++;
    }
*/
  ...
  case Modes::Clock: 
    ...
    if((modPolling>13) && checkPolling) {
      //app->SetTouchMode(DisplayApp::TouchModes::Polling);
      enableTouchBigButton= true;
      enableTouchSmallButton = true;
      checkPolling = false;
      checkTouchBigButton =false;
    } else modPolling++;
    ...
    if(checkTouchBigButton) {
        ...
        lv_arc_set_end_angle(loader, 360-tempTouch*45);
        if(tempTouch>5) {
          modPolling =0; checkPolling=true;
          enableTouchBigButton= false;
          //app->SetTouchMode(DisplayApp::TouchModes::Gestures);
        }
        
      } else  {lv_obj_set_hidden(loader, true); tempTouch=0;}

    checkTouchBigButton = false;

    if((xTaskGetTickCount()-timeoutCountStartS) < 1.5*1024){checkTouchSmallButton = false;}
    if(checkTouchSmallButton) {
      tempTouchSmall++;
      lv_obj_set_hidden(buttonSmallCircle , false);
      lv_arc_set_end_angle(buttonSmallCircle , 360-tempTouchSmall*38);
    ...
    } else  {lv_obj_set_hidden(buttonSmallCircle , true); tempTouchSmall=0;}
  ...
  case Modes::Test:
      ...
      if(batteryController.PercentRemaining()<25) batteryController.sendAlerts(0x05); 
      else batteryController.sendAlerts(0x06);  
      checkTest=true;                 
    }
   ...
   if(batteryController.isTimerStart1() && (batteryController.getCheckinTime1()== batteryController.getCurrentHour()) && ( batteryController.getCurrentMinute()<3)) {             
    ...
    if((xTaskGetTickCount()-timeoutCountStartS) < 1.5*1024){checkTouchSmallButton = false;}
      if(checkTouchSmallButton) {
        tempTouchSmall++;
        lv_obj_set_hidden(buttonSmallCircle , false);
        lv_arc_set_end_angle(buttonSmallCircle , 360-tempTouchSmall*45);
        if(tempTouchSmall>5) {
          modPolling =0; checkPolling=true;
          enableTouchSmallButton = false;
          //app->SetTouchMode(DisplayApp::TouchModes::Gestures);
...
*/
    ...
    case Modes::Charging: 
        ...
        if(flag>20){  
          ...
          lv_arc_set_end_angle(arcCharging, uint16_t((batteryPercent*3.6)));
        } else flag++;     
    ...
    case Modes::Oxi:
        ...
        if(!checktemp) break;
        if(modoxi>=600) {sprintf(sp, "%d",t); lv_label_set_text(heartbeatValue, sp);lv_label_set_text(lablepercent,"%");}
        if(modoxi>601) break;
        t=98+rand()%2;
        modoxi++;      
        if((spO2<20)||(spO2>100)) {
        if(std::fmod(modoxi/5,5))
        lv_label_set_text(heartbeatValue, "--");
        else lv_label_set_text(heartbeatValue, "");
        }
...
bool ScreenDisplay::OnTouchEvent(Watch::Applications::TouchEvents event) {
   switch (event) {
    ...
    case TouchEvents::SwipeLeft: {
     ...
     case TouchEvents::SwipeUp:{ return false;}
    default: {
      return false;
    }
  } 
