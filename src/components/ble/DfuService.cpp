...
DfuService::DfuService(Watch::System::SystemTask& systemTask,
  ...
  timeoutTimer = xTimerCreate("notificationTimer", 80000, pdFALSE, this, TimeoutTimerCallback);
}
  ...
  switch (opcode) {
    case Opcodes::StartDFU: {
      ...
      if (imageType == ImageTypes::Application) {
        ...
        systemTask.PushMessage(Watch::System::Messages::BleFirmwareUpdateStarted);
        return 0;
      } else {
...
void DfuService::Reset() {
  ...
  systemTask.PushMessage(Watch::System::Messages::BleFirmwareUpdateFinished);
}
